...
const ViewTaskScreen = () => {
  ...
  const [reason, setReason] = useState(taskInfo?.remark ?? "");
  ...
  const [absentList, setAbsentList] = useState(taskInfo?.absent_list ?? []);

  useEffect(() => {
    console.log("Item: ", taskInfo);
    if (absentList.length > 0) {
      supabase_api.shared
        .getStudentInfoFromList(absentList)
        .then((res) => {
          setSearchList(res);
        })
        .catch((error) => {
          ErrorLogger.shared.ShowError(
            "ViewTaskScreen: getStudentInfoFromList: ",
            error
          );
        })
        .finally(() => {
  ...
  const renderClassDetails = () => {
    ...
    const {
      ...
      subject_info: {
        subject,
        class_info: { standard: standard, section: section },
      },
    } = taskInfo;
    ...
    return (
      ...
      >
        ...
        />
        <InformationView label={"Slot"} value={slot} />
        <InformationView
          ...
          <View>
            <View style={styles.toolbarView}>
              <View style={styles.headerView}>
                ...
                <Text style={styles.headerText}>View Task</Text>
              </View>
              <Text style={styles.subText}>
                View out class details, topics covered, track attendance, and
                remarks.
              </Text>
            </View>
              ...
              >
                <View style={styles.inputView}>
                  ...
                  {searchList.length > 0 ? (
                    ...
                    >
                      {searchList.map((searchItem) => renderUser(searchItem))}
                    </View>
                  ) : (
                    <TouchableOpacity
                      disabled
                    ...
                    >
                      <ButtonItem>
                        <ButtonLabel>{"No Student Absent"}</ButtonLabel>
                      </ButtonItem>
                    </TouchableOpacity>
                  )}
                </View>
                <View style={styles.inputView}>
                  ...
                  >
                    <ButtonItem>
                      <ButtonLabel style={{color: secondaryText}}>{"No Topic Selected"}</ButtonLabel>
                    </ButtonItem>
                  </TouchableOpacity>
                </View>
                <View style={styles.inputView}>
                  ...
                  {reason?.length === 0 ? (
                    ...
                    >
                      <ButtonItem>
                        <ButtonLabel style={{color: secondaryText}}>{"No Remark"}</ButtonLabel>
                      </ButtonItem>
                    </TouchableOpacity>
                  ) : (
                    <TextInput
...
export default React.memo(ViewTaskScreen);
